...
@app.route("/inavtar")
def inavatar():
    return render_template("Age.html", title="Login", message="Welcome back! Please login 🌟")
...
@app.route('/3years')
def years3():
    return render_template('age3.html')
@app.route('/videosof3')
def vivdeosof3():
    return render_template('video.html')
@app.route('/rhyme')
def rhymess():
    return render_template('rhy3.html')
@app.route('/m2')
def pazzuls():
    return render_template('m2.html')
@app.route('/colour')
def colour():
    return render_template('colour3.html')
@app.route('/story')
def story():
    return render_template('stories3.html')
@app.route('/solar')
def solar():
...
@app.route('/8years')
def yearsof8():
    return render_template('8y.html')
...
def tus():
    return render_template('simplequiz.html')
@app.route('/about')
def abt():
...
def all_users():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("SELECT fullname, email, username, password,profile_pic FROM users")
    ...
    for u in users:
        result += f"<li>{u['fullname']} | {u['email']} | {u['username']} | {u['password']} |{u['profile_pic']}</li>"
...
def login():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            flash("❌ Invalid login. Please register!", "danger")
            return redirect(url_for("log"))

    return render_template("login1.html")
@app.route("/inavtar1", methods=["GET", "POST"])
def inavata1():
    ...
    if request.method == "POST":
        ...
        return jsonify({"status": "success", "avatar": avatar, "avatar_name": avatar_name})
...
def set_avatar():
    if "username" not in session:
        return {"status": "error", "msg": "Login required"}, 401

    avatar_url = request.json.get("avatar")
    if not avatar_url:
        return {"status": "error", "msg": "No avatar provided"}, 400

    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET profile_pic=%s WHERE username=%s", (avatar_url, session["username"]))
    conn.commit()
    cursor.close()
    conn.close()

    return {"status": "success", "msg": "Avatar updated"}
...
def profile():
    ...
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True, buffered=True)
    cursor.execute("SELECT fullname, email, username ,profile_pic FROM users WHERE username=%s", (username,))
    user = cursor.fetchone()
    cursor.close()
    conn.close()
    ...
    # ✅ Pass DB user data into template
